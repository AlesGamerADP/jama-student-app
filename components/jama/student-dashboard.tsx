...
interface Props {
  platos: Plato[]
  pedidos: Pedido[]
  onReservar: (plato: Plato, entrada: string, metodoPago: MetodoPago) => Pedido
  onLogout: () => void
}
...
export function StudentDashboard({
  ...
  function confirmarReserva() {
    if (!selectedPlato || !selectedEntrada) {
      notify({
        tone: "info",
        title: "Campos incompletos",
        message: "Debes seleccionar una entrada.",
      })
      return
    }
    ...
    setTimeout(() => {
      const pedido = onReservar(selectedPlato, selectedEntrada, metodoPago)
      setTicket(pedido)
...
function ReservaModal({
  ...
  const entradas = getEntradas(plato.restaurante)
